function results=noiseCorr(rates_mean,DataSet,bin_size,laboratories,trials_lim,name,doPlotLines,initial_to_plot,n_bins_x_trial)
%Inputs:
...
disp('Noise corr. interval-discrimination task')
...
%
tone1_1=round(nanmean(all_tone1_1));if tone1_1<1, tone1_1=1; end
tone1_2=round(nanmean(all_tone1_2));if tone1_2<2, tone1_2=2; end
shorts2_1=all_tone2_1(all_tone2_1<nanmean(all_tone2_1));
shorts2_2=all_tone2_2(all_tone2_2<nanmean(all_tone2_2));
tone2_1=round(nanmean(shorts2_1));
tone2_2=round(nanmean(shorts2_2));

long_tone1_1=tone1_1;
long_tone1_2=tone1_2;
longs2_1=all_tone2_1(all_tone2_1>nanmean(all_tone2_1));
longs2_2=all_tone2_2(all_tone2_2>nanmean(all_tone2_2));
long_tone2_1=round(nanmean(longs2_1));
long_tone2_2=round(nanmean(longs2_2));

movement=round(mean(all_movement));


%There is no much point in plotting way beyond the offset of the second
%tone. We need to fix a number of bind per trial, one possibility
%is that the first trial is the guide, another is that is just the mean.
%Anyway it does not make difference for the sake of the plot because the
%second tone offset always occurs very early in the trial.
...
total_order_2_corr=zeros(n_bins_x_trial,(n_units^2)-n_units);%These are the number of different corr coeffs. One per bin.
...
%                 
                ...
                numerator=mr1*mr2';
                denominator=( (mr1*mr1')* (mr2*mr2') ).^(1/2);
                %order_2_corr(current_bin)=numerator/denominator;%positive
                total_order_2_corr(current_bin,t)=numerator/denominator;
                
            end
            
            subplot(211)
            values=total_order_2_corr(:,t);
...
%figure
subplot(223)
values=nanmean(abs(total_order_2_corr),2);
devs=nanstd(abs(total_order_2_corr),0,2)/sqrt(t);if (length(times)==length(values))&&(length(times)==length(devs))&&((length(times)+length(values)+length(devs))>0)
...
xlabel('Time (ms)'), ylabel('Correl.'),title('abs values'),

subplot(224)
values=nanmean((total_order_2_corr),2);
devs=nanstd((total_order_2_corr),0,2)/sqrt(t);
if (length(times)==length(values))&&(length(times)==length(devs))&&((length(times)+length(values)+length(devs))>0)
shadedErrorBar(times,values,devs,...
    {'Color',[0 0 0.7],'Linewidth',2})
end
...
xlabel('Time (ms)'), ylabel('Correl.'),title('real values'),


set(gcf,'Name',name)

results.noisecorrel=total_order_2_corr;
