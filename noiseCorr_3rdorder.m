function results=noiseCorr_3rdorder(rates_mean,DataSet,bin_size,laboratories,trials_lim,name,doPlotLines,initial_to_plot,n_bins_x_trial,do_plots)
%Inputs:
...
%Outputs:
%    3rd order noise correlation. See code
%
...
if nargin<10, do_plots=0; end
addpath('shadedErrorBar');
disp('Noise corr. triplets interval-discrimination task')
...
for unit1=1:n_units
        ...
        for unit3=unit2:n_units
            ...
            if ~((unit2==unit1)&&(unit2==unit3))
                ...
                for current_bin=1:n_bins_x_trial
                    ...
                    numerator=sum(mr1.*mr2.*mr3);
                    % Warning the commented one does not normalize to
                    %one (is the 'physics' definition)
                    %                     denominator=(  sum(mr1.^2) * sum(mr2.^2) * sum(mr3.^2)  ).^(1/2);
                    denominator=(( sum(mr1.^3)* sum(mr2.^3)* sum(mr3.^3) ).^(1/3));
                    if isreal(denominator) && ( abs(numerator/denominator)<0.9 )
    ...
    if do_plots
        ...
        xlabel('Time (ms)'), ylabel('3rd order correl.'),title('abs values'),
        ...
        xlabel('Time (ms)'), ylabel('3rd order correl.'),title('real values'),
