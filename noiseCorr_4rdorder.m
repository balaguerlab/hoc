function results=noiseCorr_4rdorder(rates_mean,DataSet,bin_size,laboratories,trials_lim,name,doPlotLines,initial_to_plot,n_bins_x_trial)
%Inputs:
...
%Outputs:
%    4rd order noise correlation. See code
%
...
disp('Noise corr. quatruplets interval-discrimination task')
...
for unit1=1:n_units
    ...
    for unit2=unit1:n_units
        ...
        for unit3=unit2:n_units
            ...
            for unit4=unit3:n_units
                ...
                if ~((unit1==unit2)&&(unit2==unit3)&&(unit3==unit4))
                    t=t+1;
                    for current_bin=1:n_bins_x_trial
                        ...
                        numerator=sum(mr1.*mr2.*mr3.*mr4);
                        %Warning the commented one does not normalize to
                        %one (is the 'physics' definition
                        %denominator=( (mr1*mr1')* (mr2*mr2')*(mr3*mr3')*(mr4*mr4') ).^(1/2);
                        denominator=(( sum(mr1.^4)* sum(mr2.^4)* sum(mr3.^4) * sum(mr4.^4) ).^(1/4));
    ...
    xlabel('Time (ms)'), ylabel('4th order correl.'),title('abs values'),
    ...
    xlabel('Time (ms)'), ylabel('4th order correl.'),title('real values'),
    set(gcf,'Name',name)
